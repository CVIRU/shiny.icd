...
icd_package_procedure_icd9_v1 <- function() {
  head(icd.data::icd9cm_billable[[1]])
  names(icd.data::icd9cm_billable)
  # "32" "31" "30" "29" "28" "27" "26" "25" "24" "23"

  class(icd.data::icd9cm_billable)
  class(icd.data::icd9cm_billable$`32`)
  # data.frame
  colnames(icd.data::icd9cm_billable[[1]])
  ...
  # Define hierarchy
  # Source: http://www.icd9data.com/2012/Volume3/default.htm
  head(icd9cm_hierarchy)
  class(icd9cm_hierarchy)

  # Use Version 30 only for now (2012)----
  icd9cm_sg_hierarchy <- icd9cm_sg_billable$`30`
  icd9cm_sg_hierarchy$sub_code <- substr(x = icd9cm_sg_hierarchy$code,
                                         start = 1,
                                         stop = 2)
  mapping <- data.frame(sub_code = unique(icd9cm_sg_hierarchy$sub_code),

# |----------------------------------------------------------------------------------|
# | Project: Example of using package 'icd'                                         |
# | Script: Example of using package 'icd'                                          |
# | Authors: Davit Sargsyan                                                          |
# | Created: 03/31/2018                                                              |
# |----------------------------------------------------------------------------------|

icd_package_example <- function() {
  ...
  length(icd.data::icd9_majors)

  dt1 <- data.table(diagnosis = names(icd.data::icd9_majors),
                    icd9 = icd.data::icd9_majors)

  dt1 <- data.table(diagnosis = names(icd.data::icd9_chapters),
                    icd9 = icd.data::icd9_chapters)
  dt1$diagnosis
  write.csv(dt1,
            file = "tmp/dt1.csv")

  # Billable ICD9 codes, revisions 23 to 32
  dt1 <- icd.data::icd9cm_billable
  names(dt1)
  head(dt1[[6]])
  dt1 <- rbindlist(dt1)
